...
export default function OrdersPage() {
  ...
  const [menuMapByRestaurant, setMenuMapByRestaurant] = useState<Record<string, Record<string, string>>>({})
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        ...
        const needRestaurantNames = fetchedOrders.some((o) => !o.restaurantName)
        if (needRestaurantNames) {
          try {
            ...
            fetchedOrders = fetchedOrders.map((o) => ({
              ...o,
              restaurantName: o.restaurantName || (restoMap.get(o.restaurantId) as string) || "Restoran",
            }))
          } catch {
        ...
        try {
          const uniqueRestaurantIds = Array.from(new Set(fetchedOrders.map((o) => o.restaurantId)))
          const details = await Promise.all(
            uniqueRestaurantIds.map(async (rid) => {
              try {
                ...
                const menuArray = Array.isArray(restoData?.menu) ? restoData.menu : []
        ...
        fetchedOrders.sort((a: Order, b: Order) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
        ...
        {orders.length === 0 ? (
          <Card>
            <CardContent className="pt-12 pb-12 text-center">
              ...
              <p className="text-sm text-muted-foreground">Pesan makanan favorit Anda sekarang!</p>
            </CardContent>
          </Card>
        ) : (
          <div className="space-y-4">
                ...
                <CardContent className="pt-6">
                  <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-center">
                    ...
                    <div>
                      ...
                      <ul className="font-medium space-y-1">
                        {order.items.map((it, idx) => {
                          const names = menuMapByRestaurant[order.restaurantId] || {}
                    ...
                    <div>
                      ...
                      <p className="font-bold text-primary">{formatIDR(order.totalPrice)}</p>
                    </div>
                    <div>
